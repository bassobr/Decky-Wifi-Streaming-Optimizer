const manifest = {"name":"WiFi Optimizer Streaming"};
const API_VERSION = 2;
...
const setStreamingMode = callable("set_streaming_mode");
        ...
        : diagState === "saved"
            ...
            : diagState === "error"
                ...
                : diagState === "copying"
                    ? "Collecting..."
                    : "Copy diagnostics";
    return (SP_JSX.jsxs(DFL.PanelSection, { children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs("div", { style: rowStyle, children: ["v", version, " - by jasonridesabike"] }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs("div", { style: rowStyle, children: ["If WiFi won't reconnect, a reboot usually fixes it.", SP_JSX.jsx("br", {}), "Bugs? Report at github.com/bassobr/Decky-Wifi-Streaming-Optimizer"] }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: diagState === "copying", onClick: handleCopyDiagnostics, children: label }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx("div", { style: rowStyle, children: "Diagnostics include network identifiers (SSID, MAC/BSSID) - review before sharing." }) })] }));
}
...
function StreamingSection({ status, isBusy, error, onToggleMode, onToggleApp, onSavePatterns, }) {
    ...
    const [patternsInput, setPatternsInput] = SP_REACT.useState(s?.streaming_custom_patterns ?? "");
    SP_REACT.useEffect(() => {
        getStreamingApps()
            .then((r) => setApps(r.apps ?? []))
            .catch(() => { });
    }, []);
    return (SP_JSX.jsxs(DFL.PanelSection, { title: "Streaming auto mode", children: [SP_JSX.jsx(InfoRow, { label: "Only fix while streaming", subtitle: enabled
                    ? "Fixes apply automatically while a streaming app runs"
                    : "Off: enabled fixes apply all the time", explanation: "Watches for the streaming apps selected below. When one starts, the lag-spike fixes you've enabled (power save, buffer tuning, CAKE) are applied automatically; when it exits, the system returns to stock settings. This keeps the battery savings of WiFi power management outside of streaming sessions. Settings that reconnect WiFi (BSSID lock, band, DNS, IPv6) stay global and are not touched mid-session.", checked: enabled, disabled: isBusy, error: error, onChange: onToggleMode }), enabled && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx("div", { style: {
                                fontSize: theme.fontSize.small,
                                color: active ? theme.success.text : theme.text.tertiary,
                                padding: "2px 0",
                            }, children: active
                                ? `● Streaming detected: ${detectedApp}`
                                : "○ No streaming app running - stock settings" }) }), apps.map((app) => (SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: SP_JSX.jsx("span", { style: { fontSize: theme.fontSize.body }, children: app.label }), checked: s?.streaming_apps?.[app.id] ?? true, disabled: isBusy, onChange: (val) => onToggleApp(app.id, val) }) }, app.id))), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.TextField, { label: "Custom process patterns (space-separated)", value: patternsInput, onChange: (e) => setPatternsInput(e.target.value), onBlur: () => onSavePatterns(patternsInput) }) })] }))] }));
}
...
class ErrorBoundary extends SP_REACT.Component {
    ...
    render() {
        if (this.state.err) {
            return (SP_JSX.jsx(Banner, { variant: "error", children: "WiFi Optimizer hit an unexpected error. Close and reopen the panel to recover. If it keeps happening, please report at github.com/bassobr/Decky-Wifi-Streaming-Optimizer." }));
        }
...
function Content() {
    ...
    const dnsEditingRef = SP_REACT.useRef(false);
    ...
    const handleStreamingMode = (val) => handleToggle("streaming_mode", () => setStreamingMode(val));
    const handleStreamingApp = (appId, val) => handleToggle("streaming_mode", () => setStreamingApp(appId, val));
    const handleStreamingPatterns = (patterns) => handleToggle("streaming_mode", () => setStreamingCustomPatterns(patterns));
    ...
    const confirmResetSettings = () => {
        if (busyRef.current)
            return;
        DFL.showModal(SP_JSX.jsx(DFL.ConfirmModal, { strTitle: "Reset Settings", strDescription: "Reverts the runtime optimizations and deletes the plugin's settings. " +
                "Per-network changes (BSSID lock, band, DNS, IPv6) stay on saved " +
                "networks until you forget and rejoin them.", strOKButtonText: "Reset", onOK: () => {
...
var index = definePlugin(() => {
    ...
    return {
        ...
        titleView: SP_JSX.jsx("div", { className: DFL.staticClasses.Title, children: "WiFi Optimizer Streaming" }),
